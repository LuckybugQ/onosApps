...
import { LogService, FnService, KeysService, SvgUtilService, PrefsService, WebSocketService, IconService, LionService, ZoomUtils, KeysToken, TopoZoomPrefs } from '../../../../gui2-fw-lib/src/public_api';
...
export class TopologyComponent implements OnInit, OnDestroy {
    ...
    constructor(
        ...
        this.is.loadIconDef('m_satellite');
    ...
    toObliqueView() {
        ...
        time = 1500,
        fill = {
            l1: 'rgba(130,130,170,0.3)', // blue-ish
            l2: 'rgba(170,130,170,0.3)', // magenta-ish
            l3: 'rgba(130,170,130,0.3)', // magenta-ish
        };
        var box = d3.select('#onos-forcesvg').node().getBBox()
        this.log.debug(box);
        var plane = this.plane
        var node = d3.selectAll('.node')
        var link = d3.selectAll('.link')
        var nodes = this.force.graph.nodes;
        var links = this.force.graph.links;
        var ox = box.x + box.width / 2;
        var oy = box.y + box.height / 2;
        padBox(box, pad);
        if(!this.oblique){
            this.oblique = true;
            insertPlanes('l1',ox, oy);
            insertPlanes('l2',ox, oy);
            insertPlanes('l3',ox, oy);
            showPlane('l1',box,-1)
            showPlane('l2',box,0)
            showPlane('l3',box,1)
            nodes.forEach(function (d){
                var oldxy = { x: d.x, y: d.y },
                    coords = xffn(oldxy, d.rank);
                d['oldxy'] = oldxy;
                d.location.longOrX = d.x;
                d.location.latOrY = d.y;
                d.x = coords.x;
                d.y = coords.y;
            });
        }else{
            ...
            nodes.forEach(function (d) {
                ...
                d.location.longOrX = old.x;
                d.location.latOrY = old.y;
                d.x = old.x;
                d.y = old.y;
                delete d['oldxy'];
            });
        ...
        node.data(nodes)
        link.data(links)
        node.transition().duration(time).attr('transform',function(d) {return 'translate('+d.x+','+d.y+')';});
        link.transition().duration(time).attr('x1',function(d) {return d.source.x;}).attr('x2',function(d) {return d.target.x;}).attr('y1',function(d) {return d.source.y;}).attr('y2',function(d) {return d.target.y;});
        ...
        function noXform() {
            return that.sus.skewX(0) + that.sus.translate([0,0]) + that.sus.scale(1, 1);
        }
        function showPlane(tag, box, dir) {
            // set box origin at center..
            box.x = -box.width/2;
            box.y = -box.height/2;
            plane[tag].select('rect').attr('x',box.x).attr('y',box.y)
        ...
        function  insertPlanes(tag,ox, oy) {
            var id = tag,
                g = d3.select('#topo-zoomlayer').insert('g', '#topo-G')
                    .attr('id', id)
        ...
        function obXform(h, dir) {
            var yt = ytfn(h, dir);
            return that.sus.scale(1, ysc) + that.sus.translate([0,yt]) + that.sus.skewX(xsk);
        }

        function ytfn(h, dir) {
            return h * ysc * dir * 1.1;
        }
        function xffn(xy, dir) {
